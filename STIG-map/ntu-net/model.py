...
class CrossModule(nn.Module):
    def __init__(self, num_class, in_channels, anchors=8, out_channels=256):
        super(CrossModule, self).__init__()
        self.num_class = num_class
        #  DFPS = True, attention=True
        anchors = int(anchors)
        self.CP1 = CrossPointNet(in_channels, anchors, int(512/anchors), [36, 64],  [[32, 32, 64], [32, 32, 64]], int(1024/anchors), out_channels, DFPS=True, time_encoding=True)
        """
         in_channels = out_channels + n_per_sample
        """
        #  attention = True
        self.fe2 = PointFeatureNet(64, [56, 128], 256+out_channels, [[64, 128, 256], [128, 256, 512]])
        # self.fe2 = PointFeatureNet(128, [256], 192+64, [[128, 128, 256]])
        self.fe3 = PointNetSetAbstraction(640+128+3, [256, 1024])
...
if __name__ == '__main__':
    # data = PointCloudDataset(args={'num_points': 2048}, cls_num=4, root="datasets/data", split='train')
    data = NtuDataset(root='data/ntu/save')
    # data = NtuDataset(root='ntu/datasets/nturgb+d_skeletons')
    model = CrossModule(120, 2)
    ...
    for all_points, points_frames, label  in DataLoader:
        ...
        # rotate_and_scale_point_cloud_x_axis(all_points, points_frames)
        # end_time = time.time()
        # print(end_time - start_time)
        # all_points = all_points.to(device)
        # points_frames = points_frames.to(device)
        # label = label.to(device)
        x = model.forward(all_points, points_frames)
