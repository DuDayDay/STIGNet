...
import argparse
import os
from datasets_msr import MSRAction
"""
    if you use no_frames please use datasets_msr_noframes 
"""
...
def parse_args():
    ...
    # 添加参数
    parser.add_argument('--data_path', type=str, default='msr/Depth', help='Path to the dataset')
    parser.add_argument('--batch_size', type=int, default=6, help='Batch size for training and testing')
    parser.add_argument('--num_class', type=int, default=20, help='Number of classification categories')
...
def train():
        ...
        train_dataset = MSRAction(root=data_path, frame_clip=16, num_points = 256)
        test_dataset = MSRAction(root=data_path, frame_clip=16, num_points = 256, is_test=True)
        ...
        if args.pretrained:
            try:
                ...
                # print(classifier.load_state_dict(checkpoint['optimizer_state_dict']))
        ...
        for epoch in range(start_epoch, epochs):
            ...
            for batch_id, (points, points_frames, target) in tqdm(enumerate(train_loader), total=len(train_loader)):
                optimizer.zero_grad()
                """
                    if you use no_frames please use datasets_msr_noframes please annotation below
                """
                "----------------------------------------------------------------------------------"
