...
def find_nearest_neighbors(sample_points, points_frames, K):
    ...
    # 构造帧索引掩码，提取每个点对应的帧数据
    frame_indices = sample_frames.unsqueeze(-1).unsqueeze(-1).expand(-1, -1, P, 3)  # [B, N, P, 3]
    ...
    dists = torch.cdist(sample_coords_expanded, selected_points_frames, p=2)  # [B, N, P]
    ...
    # 根据最近邻索引提取点云数据
    nearest_points = torch.gather(selected_points_frames, 2, nearest_indices_expanded.expand(-1, -1, -1, 3))

    # 合并帧信息
    nearest_neighbors[:, :, :, :3] = nearest_points - sample_coords_expanded  # 最近点的坐标 [B, N, K, 3]
...
def process_mask(a,sampled_center,num=1000):
    ...
    # 获取每个节点的最后一个有效索引
    last_idx = (counts_per_node - 1).unsqueeze(-1)
    last_valid_index = selected_indices.gather(dim=-1, index=last_idx)
    # 替换超出有效数量的位置
    final_indices = torch.where(replace_mask, sampled_center.unsqueeze(-1), selected_indices)

    return final_indices
def get_nearest_points_within_radius_parallel(sampled_center, points_tensor, n_per_sample=64):
    """
    ...
    neighbors_idx = process_mask(within_radius,sampled_center, num=2000)
    gathered_points = all_points[torch.arange(B).unsqueeze(1).unsqueeze(2), neighbors_idx]
    gathered_points_flat = gathered_points.view(B*S, 2000, 3)
    FPS_gathered_points = farthest_point_sample(gathered_points_flat, n_per_sample)
...
class PointFeatureNet(nn.Module):
    ...
    def forward(self, xyz, features):
        ...
        xyz_new = index_points(xyz, farthest_point_sample(xyz, self.n_points))
        new_features_list = []
...
class CrossPointNet(nn.Module):
    def __init__(self, in_channels, num_sample, global_sample, kernel_list, mlp_list, n_per_sample=64, out_channels=128, DFPS = False, time_encoding = True):
    ...
    def forward(self,points,points_frames):
        ...
        if self.DFPS is True:
            _,frames,_,_ = points_frames.shape
        elif self.DFPS is False:
            frames = int(points_frames[1])
        else:
            print('WARNING: the input and the state of the DFPS is erro')

        sampled_points_idx = farthest_point_sampling_with_density(points, self.num_sample)
        sampled_center, neighbors =get_nearest_points_within_radius_parallel(sampled_points_idx, points,n_per_sample=self.n_per_sample)
        ...
        for i, kernel in enumerate(self.kernel_list):
            if self.DFPS is False:
                ...
                grouped_features[..., :3] = grouped_features[..., :3] - anchor_xyz.view(B,
                                                                                        self.num_sample * self.n_per_sample,
                                                                                        1, 3)
                if self.time_encoding:
                    grouped_features = time_encoding(grouped_features, frames)
            else:
                nearest_neighbors = find_nearest_neighbors(sampled_points_concat, points_frames, kernel)
                group_idx, _, dist, _ = closest_points(kernel, xyz, sampled_points_concat[:, :, 0:3])
                grouped_features = index_points(points, group_idx)
                anchor_xyz = sampled_points_concat[..., :3]
            ...
            grouped_features = grouped_features.permute(0, 3, 2, 1)
            if self.DFPS is False:
                mix_feature = torch.max(grouped_features, dim=2)[0]
        ...
        # visualize_point_cloud(np.array(points[]), np.array(idx[0]))
